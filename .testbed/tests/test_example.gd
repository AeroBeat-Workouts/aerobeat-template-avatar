...
const EXPECTED_PLUGIN_DESCRIPTION := "Template for AeroBeat internal avatar repos. Shared avatar resources built on aerobeat-asset-core for current v1 work."
const EXPECTED_TESTBED_NAME := "AeroBeat Internal Avatar Testbed"
...
func test_readme_keeps_v1_internal_avatar_template_truth() -> void:
	var readme_text := _read_repo_file(README_PATH)
	assert_true(readme_text.contains("official template for creating **internal avatar** repositories"), "README should state that this repo is an internal avatar template")
	...
	assert_true(readme_text.contains("internal/system avatar work"), "README should preserve the downscoped internal-only avatar stance")
	assert_true(readme_text.contains("UGC or package-local gameplay avatar swap story"), "README should explicitly reject the old UGC/package-local avatar framing")
...
func test_plugin_cfg_description_stays_template_specific() -> void:
	...
	assert_eq(config.get_value("plugin", "name", ""), "AeroBeat Internal Avatar Template", "plugin.cfg name should stay stable")
	assert_eq(
		config.get_value("plugin", "description", ""),
		EXPECTED_PLUGIN_DESCRIPTION,
		"plugin.cfg description should remain aligned with the template's narrow v1 avatar-lane contract"
	)
...
func test_hidden_testbed_name_stays_internal_template_specific() -> void:
	...
	assert_eq(
		...
		"hidden workbench name should stay aligned with the internal avatar template contract"
	)
